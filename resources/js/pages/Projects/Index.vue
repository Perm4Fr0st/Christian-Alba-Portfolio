...
async function confirmDelete() {
    ...
    router.delete(route('projects.delete', projectToDelete.value.id), {
        preserveScroll: true,
        onSuccess: () => {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div class="flex items-center justify-between">
                <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">All Projects</h1>
                <a 
            ...
            <div class="overflow-x-auto rounded-xl border border-gray-300 dark:border-gray-600" :key="renderKey">
                <table class="table-auto w-full border-collapse bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
                    ...
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left border-r border-gray-300 dark:border-gray-600">Link</th>
                    ...
                    <tbody>
                        <template v-if="projects?.data?.length > 0">
                            <tr v-for="(project, index) in projects.data" :key="project.id" class="border-t border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800">
                                <th class="px-4 py-2 text-left border-r border-gray-300 dark:border-gray-600">{{ (projects?.current_page - 1) * projects?.per_page + index + 1 }}</th>
                                <td class="px-4 py-2 border-r border-gray-300 dark:border-gray-600">{{ project.name }}</td>
                                <td class="px-4 py-2 border-r border-gray-300 dark:border-gray-600">{{ project.link }}</td>
                                <td class="px-4 py-2 border-r border-gray-300 dark:border-gray-600">
                                    <img :src="`/storage/${project.image}`" alt="Project image" class="h-10 w-auto object-contain" />
                                </td>
                        ...
                        <template v-else>
                            <tr class="border-t border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800">
                                <td colspan="5" class="px-4 py-2 text-center text-gray-500 dark:text-gray-400">Project table is empty </td>
                            </tr>
                        </template>
                    </tbody>
                </table>
